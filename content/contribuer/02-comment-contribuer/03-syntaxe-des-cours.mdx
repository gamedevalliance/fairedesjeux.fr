...
En Markdown, vous pouvez utiliser `*` ou `_` pour l'italique et le gras, selon votre préférence.
...
Pour faire parler les trois mascottes du site, entourez le dialogue du composant `Bulle`.

```html
<Bulle emote="marvin-happy">
Salut la compagnie !
</Bulle>
```
...
Pour créer une bulle neutre avec titre de votre choix, utilisez le composant `Callout`.
...
Dans le cas de vidéos de quelques secondes pour illustrer un article, vous pouvez les déposer comme une image. Préférez le format MP4, ou éventuellement WebM. Vous pouvez ensuite choisir plusieurs options :
...
- `poster = "./poster.png"` affiche une image quand la vidéo n'est pas encore lancée. Par défaut, la première image de la vidéo est affichée.
...
```html
<Video src="jelpi.mp4" autoplay muted loop />
```

```html
<Video src="demo.mp4" poster="poster.png" muted loop controls />
```
